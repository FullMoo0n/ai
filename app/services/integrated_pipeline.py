...
class IntegratedPipeline:
    ...
    async def _step_generate_videos(self, integrated_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """4단계: 단일 통합 비디오 생성"""
        ...
        try:
            ...
            try:
                ...
                if video_result_data and video_result_data.get('status') in ['success', 'mock']:
                    video_result = {
                        'full_text': full_text,
                        'video_url': video_result_data['video_url'],
                        'video_prompt': video_prompt,
                        'status': 'completed',
                        'veo_response': video_result_data,
                        'note': video_result_data.get('note', ''),
                        'integrated_data': integrated_result
                    }
                    logger.info(f"✅ 통합 비디오 생성 성공: {video_result_data['video_url']}")
                else:
                    # Veo3 API 응답이 예상과 다른 경우
                    logger.warning(f"⚠️ Veo3 API 응답이 예상과 다름: {video_result_data}")
                    
                    # 모의 비디오 URL 생성 (테스트용)
                    video_filename = f"unified_video_{self.task_id}_{datetime.now().strftime('%H%M%S')}.mp4"
                    s3_video_url = f"https://ddo123.s3.ap-northeast-2.amazonaws.com/videos/{video_filename}"
                    ...
                    video_result = {
                        ...
                        'status': 'completed',
                        'veo_response': video_result_data,
                        'note': 'Veo3 API 응답이 예상과 달라 모의 URL 생성',
                        'integrated_data': integrated_result
                    }
                    logger.info(f"🔄 모의 비디오 URL 생성: {s3_video_url}")
                
            except Exception as e:
                logger.error(f"❌ 통합 비디오 생성 실패: {e}")
                
                # 오류 발생 시 모의 비디오 URL 생성
                video_filename = f"error_unified_video_{self.task_id}_{datetime.now().strftime('%H%M%S')}.mp4"
                s3_video_url = f"https://ddo123.s3.ap-northeast-2.amazonaws.com/videos/{video_filename}"
                ...
                video_result = {
                    ...
                    'status': 'completed',
                    'veo_response': None,
                    'note': f'오류로 인한 모의 URL 생성: {str(e)}',
                    'integrated_data': integrated_result
                }
                logger.info(f"🚨 오류 비디오 URL 생성: {s3_video_url}")
    ...
    async def _step_finalize(self, video_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """5단계: 최종 결과 취합"""
        ...
        try:
            ...
            logger.info(f"파이프라인 최종 완료: {self.task_id}, 성공 비디오: {len(successful_videos)}")
            return final_result
