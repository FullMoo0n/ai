    ...
    try:
        ...
        if is_s3_url:
            ...
            if not check_s3_object_exists(s3_url):
                raise VisionS3URLError(
                    f"S3 객체를 찾을 수 없거나 접근할 수 없습니다: {s3_url}"
                )
            ...
            raise VisionS3URLError(
                f"유효하지 않은 이미지 URL 형식입니다 (S3 또는 HTTP/HTTPS 만 지원): {s3_url}"
            )

        # 4. Vision API 페이로드 구성
        if language_hints is None:
            language_hints = ["ko"]

        import httpx
        from .vision_ocr import build_payload, encode_bytes_to_b64

        # public_url에서 이미지 다운로드
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                img_response = await client.get(public_url)
                img_response.raise_for_status()
                image_bytes = img_response.content
        except Exception as e:
            raise VisionS3APIError(f"이미지 다운로드 실패 ({public_url}): {str(e)}")

        # Base64 변환 및 페이로드 생성
        image_b64 = encode_bytes_to_b64(image_bytes)
        payload = build_payload(
            image_b64, feature=feature, language_hints=language_hints
        )
        ...
        paragraphs = extract_paragraphs_spatial_proximity_advanced(
            vision_raw,
            img_w=1024,
            img_h=1024,  # 실제 이미지 크기를 알 수 없으므로 기본값 사용
            line_tol=line_tol,
            para_gap=para_gap,
...
async def batch_process_s3_images_with_vision(
    ...
    async def process_single_image(s3_url: str) -> Dict[str, Any]:
        ...
        try:
            return await process_s3_image_with_vision(
                s3_url=s3_url, feature=feature, language_hints=language_hints
            )
        except Exception as e:
    ...
    logger.info(
        f"배치 처리 완료: {len(s3_urls)}개 이미지 중 {sum(1 for r in processed_results if r.get('success', False))}개 성공"
    )
